...
namespace apriori
{
    ...
    public class FPGrowth
    {
        ...
        public FPTree Preproccess(List<Pattern> ptns)//generate headtable, 空fptree
        {
        ...
        public List<Node> SinglePath(Node T)
        {
            ...
            if (T.Children.Count == 0)//空树，叶
                return new List<Node>() { T };
            else if (T.Father != null && T.Children.Count == 1 && (ln = SinglePath(T.Children[0])) != null)//中间
                return new List<Node>() { T }.Concat(ln).ToList();
            else if (T.Father == null && T.Children.Count == 1 && (ln = SinglePath(T.Children[0])) != null)//根
                return ln;
            else
                return null;
        }

        public void Generate(List<Pattern> lp, List<Node> curln, List<Node> ln, int i, int n)//非空
        {
            if (i == n)
            {
